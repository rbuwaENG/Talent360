...
class InputValidationTest {
    ...
    fun `isSapIdValid should return true for valid sapId`() {
        val (isValid, errorMessage) = InputValidation.isSapIdValid("9915716980")
    ...
    fun `isSapIdValid should return false for empty sapId`() {
        ...
        assertThat(errorMessage).isEqualTo("NID cannot be empty.")
    }

    @Test
    fun `isSapIdValid should return false for sapId with length not equal to 10`() {
        val (isValid, errorMessage) = InputValidation.isSapIdValid("12345678")
        assertThat(isValid).isFalse()
        assertThat(errorMessage).isEqualTo("NID must be 10 characters.")
    }
    ...
    fun `isZipCodeValid should return true and an empty string for a valid state`() {
        val (isValid, message) = InputValidation.isZipCodeValid("40010")
        assertThat(isValid).isTrue()
        assertThat(message).isEmpty()
    }
